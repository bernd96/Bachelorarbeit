...
catkin build $force_color $continue_on_failure --no-status --force-cmake -G"Eclipse CDT4 - Unix Makefiles" $debug $source_project $project_arg

# run catkin again, because cmake will be forced otherwise next time and .project files will change
catkin build $force_color $continue_on_failure --no-status $project_arg
...
for PROJECT in $PROJECTS; do
	...
	python $ROOT/eclipse-setup/catkin-as-project-buildtool.py
	python $ROOT/eclipse-setup/rename_projects.py
done
cd $ROOT

echo ""
echo "CREATION OF ECLIPSE PROJECTS FINISHED! YOU CAN NOW IMPORT THEM IN ECLIPSE VIA File -> Import... -> General -> Existing Projects into Workspace"
